    ...
    private errorHandler = (): void => {
        this.app.use((error:unknown, req:Request, res:Response, next:NextFunction)=>{
            if (error instanceof AppError){
              return  res.status(error.rc).send({result:{success: error.success, message: error.message}})
            }
            return res.status(500).send(error instanceof Error? error: "Error Tidak Diketahui");
        })
